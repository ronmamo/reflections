    ...
    return new Match<Set<Class<?>>>() {
      public boolean matches(Object o) {
        for (Class<?> c : (Iterable<Class<?>>) o) {
          ...
          final Iterable<Class<? extends Annotation>> classIterable = annotationTypes(annotationList);
          if (!Iterables.contains(classIterable, annotation)) return false;
        }
        return true;
      }
    };
    ...
    return new Match<Set<Class<?>>>() {
      public boolean matches(Object o) {
        for (Class<?> c : (Iterable<Class<?>>) o) {
          ...
          final Set<Class<?>> allSuperTypes = ReflectionUtils.getAllSuperTypes(c);
          final List<Class<?>> stack = new ArrayList<>(allSuperTypes);
          while (!stack.isEmpty()) {
